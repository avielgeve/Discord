...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (isNaN(args[0])) return message.lineReply(`Please Give Me Valid ID!`);

    if (args[0] === message.author.id)
      return message.lineReply(`You Are Already Unban!`);

    if (args[0] === message.guild.owner.user.id)
      return message.lineReply(`Server Owner Is Already Unban!`);

    if (args[0] === client.user.id)
      return message.lineReply(`I Am Already Unban!`);
