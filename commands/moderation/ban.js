...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!message.guild.members.cache.get(Member.id))
      return message.lineReply(`Please Mention A Valid Member!`);

    if (Member.id === message.author.id)
      return message.lineReply(`You Can't Ban Your Self!`);

    if (Member.id === client.user.id)
      return message.lineReply(`Please Don't Ban Me ;-;`);

    if (Member.id === message.guild.owner.user.id)
      return message.lineReply(`You Can't Ban Owner Of Server!`);

    let Reason = args.slice(1).join(" ");

    let User = message.guild.member(Member);

    if (!User.bannable) return message.lineReply(`I Can't Ban That Member!`);
